...
describe('LoyaltyStampStyleService', () => {
  ...
  it('returns a default style when no explicit style exists', async () => {
    ...
    assert.equal(response.accentColor, '#facc15');
    ...
    assert.equal(response.imageAccentColor, '#facc15');
    assert.equal(response.imageTextColor, '#ffffff');
    assert.equal(response.stampFilledColor, '#facc15');
    assert.equal(response.stampEmptyColor, '#97a8e2');
    assert.equal(response.rewardBannerColor, '#2e52c4');
