...
describe('LoyaltyStampStyleService', () => {
  ...
  it('returns a default style when no explicit style exists', async () => {
    ...
    assert.equal(response.accentColor, '#fde047');
    ...
    assert.equal(response.imageSurfaceColor, '#2e52c4');
    assert.equal(response.imageAccentColor, '#fde047');
    assert.equal(response.imageTextColor, '#eff6ff');
    assert.equal(response.stampFilledColor, '#fde047');
    assert.equal(response.stampEmptyColor, '#9bace3');
    assert.equal(response.rewardBannerColor, '#0e2d93');
